...
@section('content')
    <div class="max-w-2xl mx-auto mt-6 shadow-md p-6 bg-white rounded-lg">
        <h2 class="text-xl font-semibold mb-4">Edit Vaccine record</h2>

        <form action="{{ route('vaccine_records.update',$vaccine_record->id) }}" method="POST">
            @csrf
            @method('put')
            <!-- radio -->
            <div class="mb-4">
                ...
                <select class="border" name="std_id" id="">
                    ...
                    @foreach ($students as $item)
                                       {{-- ตรงนี้มาใส่ที่หลังเพื่อเช็คว่าตรงกันไหม  --}}
                        <option {{ ($item->id == $vaccine_record->std_id) ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->fname . ' ' . $item->lname }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                @foreach ($vaccines as $item)
                    <div class="flex gap-2">
                        <input {{ ($item->id == $vaccine_record->vac_id) ? 'checked' : ' ' }} type="radio" name="vac_id" value="{{ $item->id }}" />
            ...
            <div class="mb-4">
                ...
                <input value="{{ $vaccine_record->vaccined_date }}" type="date" name="vaccined_date" id="vaccined_date"
